...
The plan describes `@treequel/provider-memory` as the reference/oracle provider that owns the in-memory execution
...
- One implementation defines the oracle semantics; the provider and the boundary can never diverge from it.
