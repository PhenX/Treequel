...
Reification has one build path: the `@greffon/vite` plugin over `@greffon/transform`, which serves Vite, Rollup and
...
A separate package **`@greffon/ts-transformer`** exposing a `before` transformer, usable through ts-patch
(`{ "transform": "@greffon/ts-transformer" }`) or the compiler API (`createTransformerFactory(program)`), with
`typescript` as a peer dependency. It is deliberately a *thin host over `@greffon/transform`*, the same way
`@greffon/vite` is — the tracer, capture and detection stay in one place so the two build paths cannot diverge.

Three sub-decisions were forced by how the compiler works:

1. **Reuse the transform through a synchronous, edit-list API.** `@greffon/transform` gained `transformModuleSync`,
...
3. **ES-module output only.** The injected `import { __expr as __tql_expr$ } from "@greffon/core"` is a live ES-module
   ...
   Greffon is ESM-only, so this costs nothing real.
...
- Reification is available to `tsc`-only projects with no bundler, closing a real gap for backends that compile
  straight to `dist/`.
- `@greffon/transform` grew a small synchronous surface (`transformModuleSync`, `planModuleSync`, `scanModuleContexts`,
  `ReifyEdit`/`ReifyPlan`, `HOST_IMPORT`). No behavior of the existing async path changed.
- `ts-patch` is the user's build tool, not a Greffon dependency; the package works equally through a hand-written
  `program.emit` driver.
- Two limitations are documented, not hidden: source maps through the transformer are coarser than the magic-string
  ...
  are acceptable under Greffon's ESM-only, bundler-first posture; the Vite plugin remains the recommended path.
