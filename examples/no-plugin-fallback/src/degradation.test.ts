...
describe("without the build plugin", () => {
  it("the in-memory provider works fully — even with a captured closure", async () => {
    ...
    const count = await memDb.users.filter((u) => u.age >= minAge).count();
    expect(count).toBe(1);
  });

  it("a remote provider parses a closure-free lambda via the runtime fallback", async () => {
    const sql = await remoteDb.users.filter((u) => u.age >= 18).explain();
  ...
  it("a remote provider rejects a closure with a teachable R3002 error", async () => {
    ...
    await expect(remoteDb.users.filter((u) => u.age >= minAge).explain()).rejects.toThrow(/R3002/);
    await expect(remoteDb.users.filter((u) => u.age >= minAge).explain()).rejects.toThrow(/minAge/);
  });
});
