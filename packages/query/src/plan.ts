...
export interface IncludeSpec {
  ...
  /**
   * Refinement of the loaded rows: `filter`/`orderBy`/`thenBy` ops applied to
...
export type PlanOp =
  | { readonly op: "filter"; readonly expr: AnyExpr }
  | { readonly op: "map"; readonly expr: AnyExpr }
 ...
 * into its target — or `undefined` once a `map`/`groupBy`/`join` reshapes
...
export function elementSource(plan: QueryPlan): string | undefined {
  ...
  for (const op of plan.ops) {
    ...
    else if (op.op === "map" || op.op === "groupBy" || op.op === "join" || op.op === "leftJoin") {
      source = undefined;
    }
  }
  return source;
